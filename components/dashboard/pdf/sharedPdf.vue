        ...
        <div class="column is-one-quarter" v-for="file in files" :key="file._id">
          ...
          <div class="card">
            ...
            <footer class="card-footer">
              <a target="_blank" :href="`${API_PDF}${file.filename}`" class="card-footer-item tooltip is-tooltip-bottom"
  ...
  export default {
    ...
    methods: {
      ...
      getMyFiles() {
        ...
        const config = {
          method: 'GET',
          url: `${keys.BASE_URL}/api/v1/files/sharedFile?id=${this.$store.state.user_details.user._id}&bucket=${keys.BUCKET_PDF}`,
          headers: {
            "Authorization": `Bearer ${this.$store.state.user_details.token}`
          }
      ...
      firstPage() {
        ...
        const config = {
          method: 'GET',
          url: `${keys.BASE_URL}/api/v1/files/sharedFile?id=${this.$store.state.user_details.user._id}&bucket=${keys.BUCKET_PDF}&page=${this.filePageNo}`,
          headers: {
            "Authorization": `Bearer ${this.$store.state.user_details.token}`
          }
        }
      ...
      searchFile() {
        ...
        const config = {
          method: 'GET',
          url: `${keys.BASE_URL}/api/v1/files/sharedFile?id=${this.$store.state.user_details.user._id}&bucket=${keys.BUCKET_PDF}&name=${this.fileSearchInput}`,
          headers: {
            "Authorization": `Bearer ${this.$store.state.user_details.token}`
          }
        }
