        ...
        <div class="column is-one-quarter" v-for="file in files" :key="file._id">
          ...
          <div class="card">
            ...
            <footer class="card-footer">
              ...
              <a @click="getFileDetailsToBeDuplicated(`${API_PDF}${file.filename}`, `${file.filename}`, `${file.metadata.name}`, `${file.metadata.tag}`)"
    ...
    methods: {
      ...
      getFileDetailsToBeDuplicated(url, filename, file_name, file_tag) {
        ...
        this.fileDuplicateUrl =
          `${keys.BASE_URL}/api/v1/files?id=${this.$store.state.user_details.user._id}&bucket=${keys.BUCKET_PDF}&tag=${file_tag}&name=${duplicateName}`
